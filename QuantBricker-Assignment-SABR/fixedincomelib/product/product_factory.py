...
class ProductFactory:
    ...
    def create_zero_spread_product(
        ...
        **kwargs: Any,
        ...
        if tenor.is_term():
            ...
            term_date = Date(
                calendar.advance(
                    value_date,
                    tenor.get_term(),
                    data_convention.business_day_convention.value,
                )
            )
        else:
        ...
        return ProductZeroSpread(term_date, data_convention.index_str, values, amount, long_or_short)
    ...
    def create_bond(
        ...
        **kwargs: Any,
        ...
        values_ = values
        if values_ <= 1.0:
            prod = ProductBond(name=axis1, bond_specs=bond_specs, trade_date=trade_date, buy_sell=buy_sell.to_string())
...
ProductBuilderRegistry().register(
    DataConventionOvernightIndexBasisSwap.type(),
    ProductFactory.create_overnight_index_basis_swap,
)
ProductBuilderRegistry().register(DataConventionZeroSpread.type(), ProductFactory.create_zero_spread_product)
